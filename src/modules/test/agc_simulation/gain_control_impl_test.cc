...
class GainControlImplTest::GainController {
 public:
...
int GainControlImplTest::instance_counter_ = 0;

GainControlImplTest::GainControlImplTest()
...
void GainControlImplTest::ProcessRenderAudio(
...
void GainControlImplTest::PackRenderAudioBuffer(
    AudioBuffer* audio,
...
int GainControlImplTest::AnalyzeCaptureAudio(AudioBuffer* audio) {
...
int GainControlImplTest::ProcessCaptureAudio(AudioBuffer* audio,
  ...
  if (mode_ == kAdaptiveAnalog && !was_analog_level_set_) {
    //std::cout << "kStreamParameterNotSetError" << std::endl;
    return kStreamParameterNotSetError;
  }

  //std::cout << "In Agc!" << enabled_  << " : " << was_analog_level_set_ << std::endl;
  //std::cout << "In Agc Process!" << was_analog_level_set_ << std::endl;
...
int GainControlImplTest::compression_gain_db() const {
  return compression_gain_db_;
}

// TODO(ajm): ensure this is called under kAdaptiveAnalog.
int GainControlImplTest::set_stream_analog_level(int level) {
...
int GainControlImplTest::stream_analog_level() {
...
int GainControlImplTest::Enable(bool enable) {
  if (enable && !enabled_) {
...
bool GainControlImplTest::is_enabled() const {
  return enabled_;
}

int GainControlImplTest::set_mode(Mode mode) {
...
GainControl::Mode GainControlImplTest::mode() const {
  return mode_;
}

int GainControlImplTest::set_analog_level_limits(int minimum, int maximum) {
  if (minimum < 0) {
    return kBadParameterError;
  }
...
int GainControlImplTest::analog_level_minimum() const {
  return minimum_capture_level_;
}

int GainControlImplTest::analog_level_maximum() const {
  return maximum_capture_level_;
}

bool GainControlImplTest::stream_is_saturated() const {
  return stream_is_saturated_;
}

int GainControlImplTest::set_target_level_dbfs(int level) {
  if (level > 31 || level < 0) {
    return kBadParameterError;
  }
  {
...
int GainControlImplTest::target_level_dbfs() const {
  return target_level_dbfs_;
}

int GainControlImplTest::set_compression_gain_db(int gain) {
  if (gain < 0 || gain > 90) {
    return kBadParameterError;
  }
  {
...
int GainControlImplTest::enable_limiter(bool enable) {
  {
...
bool GainControlImplTest::is_limiter_enabled() const {
  return limiter_enabled_;
}

void GainControlImplTest::Initialize(size_t num_proc_channels, int sample_rate_hz) {
  ...
  //std::cout << "通道数 = " << num_proc_channels_ << std::endl;
...
int GainControlImplTest::Configure() {
